...
export default function App() {
  ...
  async function load(p: string) {
    ...
    const foldersRes = await fetch(
      `http://localhost:3000/api/fdb/directory/folders?path=${clean}&recursive=false`
    );
    const filesRes = await fetch(
      `http://localhost:3000/api/fdb/directory/files?path=${clean}&recursive=false&data=false`
    );
